...
def test():
    ...
    scenario += c1
    c1.knownSecret(secret = sp.bytes("0x12345678aa")).run(sender = bob, valid = False)
    c1.knownSecret(secret = sp.bytes("0x12345678aabb")).run(sender = bob)
    scenario.h1("c2")
    scenario += c2
    scenario.h2("C2.export()")
    scenario.p(c2.export())

sp.add_compilation_target("atomicSwap", AtomicSwap(sp.mutez(12),sp.timestamp(50),sp.bytes("0x307839304336453830343039324344304138324531443132413043324538324238384544"), sp.address("tz1c3GoMtLJgd3pyodoG7BpHdXgbKYVfnK3N"), sp.address("tz1c3GoMtLJgd3pyodoG7BpHdXgbKYVfnK3N")))
